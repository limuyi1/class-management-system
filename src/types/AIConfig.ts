...
11. 返回结果中的 name 必须与输入学生姓名完全一致，不得新增、删减、改名或交换学生信息
12. 仅返回标准JSON数组，不要返回JSON以外的任何文字，不要 Markdown，不要代码块`,
...
6. 每条 comment 最终都必须保证100-120字之间，不能少于100字，也不能超过120字
7. 开头不出现学生姓名，不使用“该生”“本学生”等书面称谓
8. 每条输入都必须返回同名结果；仅返回标准JSON数组，不要返回JSON以外的任何文字，不要 Markdown，不要代码块`,
