...
3. 若无标签，可根据当前学生姓名寓意、班主任常见观察视角合理联想
4. 每条评语必须在最终输出前自行检查字数，按中文正文字符数估算，空格不计入
5. 若少于100字，必须先补充具体表现、进步或鼓励建议后再输出；若超过120字，必须先删减重复修饰和泛泛表述后再输出
...
1. 评语积极正面、温和鼓励，贴合小学生年龄特点
2. 紧密结合学生标签的个性/行为特点展开评价
3. 若无标签，可根据当前学生姓名寓意、班主任常见观察视角合理联想；姓名寓意只能用于当前 name 对应学生，不得串用到其他学生
4. 每条 comment 必须在最终返回前自行检查字数，按中文正文字符数估算，空格不计入
5. 若某条 comment 少于100字，必须先补充该学生的具体表现、进步或鼓励建议后再返回；若超过120字，必须先删减重复修饰和泛泛表述后再返回
6. 每条 comment 最终都必须保证100-120字之间，不能少于100字，也不能超过120字
7. 开头不出现学生姓名，直接以评语内容开篇，不使用”该生””本学生”等书面称谓
8. 可恰当引经据典，提升文采与教育意义
9. 每个学生都必须生成评语，即使标签为空也要生成符合其特点的鼓励性评语
10. 返回结果中的 name 必须与输入学生姓名完全一致，不得新增、删减、改名或交换学生信息
11. 仅返回JSON数组，不要有其他任何文字`,
...
3. 润色后必须在最终输出前自行检查字数，按中文正文字符数估算，空格不计入
4. 若少于100字，必须先补充具体表现、进步或鼓励建议后再输出；若超过120字，必须先删减重复修饰和泛泛表述后再输出
5. 最终输出必须保证100-120字之间，不能少于100字，也不能超过120字
6. 开头不出现学生姓名，不使用“该生”“本学生”等书面称谓
7. 只输出润色后的纯文本，不要标题、解释、Markdown 或 JSON`,
...
2. 保持积极、温和、鼓励，适合小学生期末评语
3. 每条 comment 必须在最终返回前自行检查字数，按中文正文字符数估算，空格不计入
4. 若某条 comment 少于100字，必须先补充具体表现、进步或鼓励建议后再返回；若超过120字，必须先删减重复修饰和泛泛表述后再返回
5. 每条 comment 最终都必须保证100-120字之间，不能少于100字，也不能超过120字
6. 开头不出现学生姓名，不使用“该生”“本学生”等书面称谓
7. 每条输入都必须返回同名结果；仅返回JSON数组，不要其他任何文字`,
