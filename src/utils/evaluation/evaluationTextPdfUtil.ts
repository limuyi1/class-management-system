...
/** 绘制评语格顶部的称呼（"xx同学："），返回称呼后的正文起始 Y 坐标 */
 ...
 * 后续若要调整行高、首行缩进、截断规则，优先改 `evaluationTextLayoutUtil.ts`。
 */
...
/**
 * 导出期末评语文字版 PDF：按纸张与评语卡排版、逐格绘制并触发下载。
 * @param options - 导出参数（配置、学生、可选文件名）
 * @returns 导出结果，含是否成功与发生截断的学生名单
 */
