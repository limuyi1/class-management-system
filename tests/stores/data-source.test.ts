...
describe('useDataSourceStore', () => {
  ...
  it('should calculate average correctly', () => {
    ...
    configurationStore.data.inputScoreTab = 'yu3_wen2'
  ...
  it('should get score for specific student', () => {
    ...
    const store = useDataSourceStore()
    store.data = [{ xing4_ming2: '张三', yu3_wen2: 85 }]

    const getScore = store.getScore
    expect(getScore(store.data[0])).toBe(85)
  })

  it('should return null when no score tab configured', () => {
    const configurationStore = useConfigurationStore()
    configurationStore.data.inputScoreTab = null

    const store = useDataSourceStore()
    store.data = [{ xing4_ming2: '张三', yu3_wen2: 85 }]

    const getScore = store.getScore
    expect(getScore(store.data[0])).toBeNull()
  })
